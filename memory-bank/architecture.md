...
- Canonical schema `thongbao`: dùng field `tieu_de/noi_dung/loai_thong_bao/doi_tuong_nhan` và constants `Thongbao::TYPE_*` + `Thongbao::TARGET_*` để tránh lệch key/chuỗi rải rác.
...
- `users.toa_nha_id` (nullable) dùng để gắn phạm vi tòa nhà cho tài khoản vận hành khi cần (hiện tại hệ thống vẫn chỉ có 1 cấp Admin toàn quyền).
...
- Quy ước đăng ký Observer: tập trung tại `AppServiceProvider::boot()` để dễ audit, tránh bị phân tán sang nhiều provider.
