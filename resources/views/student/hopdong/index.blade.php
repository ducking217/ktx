...
@section('noidung')
    ...
    @elseif($activeTab === 'gia-han' && !$isAlumni)
        <div class="space-y-8">
                ...
                <div class="saas-card p-6 bg-rose-50/30 border-rose-100/60">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <button
                            ...
                            @if($trangThaiTraPhong === \App\Enums\RegistrationStatus::Pending->value) disabled @endif
                        >
                            Yêu cầu trả phòng
                        </button>
                    </div>

                    @if($trangThaiTraPhong === \App\Enums\RegistrationStatus::Pending->value)
                    ...
                    @elseif($trangThaiTraPhong === \App\Enums\RegistrationStatus::Rejected->value)
                    ...
                    <article id="form-gia-han" class="saas-card p-8">
                        ...
                        <form action="{{ route('student.giahan.store') }}" method="POST" class="mt-6 space-y-6" @if(!$hasActiveContract) data-no-loading="true" @endif>
                            ...
                            <div class="space-y-2">
                                <div class="grid gap-4 sm:grid-cols-2">
                                    <div class="space-y-1">
                                        ...
                                        >
                                            <option value="3" @selected($goiMacDinh === 3)>3 tháng</option>
                                            <option value="5" @selected($goiMacDinh === 5)>5 tháng (01 học kỳ)</option>
                                            <option value="6" @selected($goiMacDinh === 6)>6 tháng</option>
                                            <option value="12" @selected($goiMacDinh === 12)>12 tháng</option>
                                        </select>
                                    </div>
                            ...
                            <div class="flex items-center justify-end gap-3 pt-2">
                                <a href="{{ route('student.hopdong.index', ['tab' => 'hopdong']) }}" class="saas-btn-secondary h-10 px-4 text-xs font-semibold">Quay lại</a>
                        ...
                        <script>
                            document.addEventListener('DOMContentLoaded', function () {
                                ...
                                const base = @js(($ngayKetThuc?->format('Y-m-d')) ?: now()->format('Y-m-d'));
            ...
            @push('modals')
                <x-modal id="modal-traphong" title="Yêu cầu trả phòng" subtitle="Nhập lý do trả phòng để Ban quản lý xem xét trước khi thanh lý hợp đồng.">
                    <form action="{{ route('student.yeucautraphong') }}" method="POST" class="space-y-6" onsubmit="return confirm('Gửi yêu cầu trả phòng? Ban quản lý sẽ xem xét trước khi thực hiện thanh lý.')">
                        @csrf
                        <div class="space-y-2">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">{{ $item->phong->toa ?? 'Chưa có' }} • Tầng {{ $item->phong->tang ?? 'Chưa có' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
