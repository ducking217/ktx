...
@section('noidung')
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <a href="{{ route('student.thongbao') }}" class="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-slate-900 transition-colors group">
                ...
                <article class="saas-card p-10 bg-white">
                    <header class="mb-10 pb-10 border-b border-slate-100">
                        <div class="flex items-center gap-3 mb-6">
                            ...
                            <time class="text-[10px] font-bold text-slate-400 uppercase tracking-widest tabular-nums">Đăng ngày {{ $thongbao->created_at?->format('d/m/Y') }}</time>
                        </div>
                        <h1 class="text-3xl font-black text-slate-900 tracking-tight leading-tight">{{ $thongbao->tieu_de }}</h1>
                    </header>
            ...
            <aside class="lg:col-span-4 space-y-8">
                ...
                <article class="saas-card overflow-hidden">
                    ...
                    <div class="divide-y divide-slate-50">
                        @forelse($thongbaoLienQuan as $tb)
                            <a href="{{ route('student.chitietthongbao', $tb->id) }}" class="group block p-5 transition-colors hover:bg-slate-50/50">
                                <time class="text-[9px] font-bold text-brand-emerald uppercase tracking-widest mb-1.5 block tabular-nums">{{ $tb->created_at?->format('d/m/Y') }}</time>
                                <h4 class="text-xs font-bold text-slate-900 leading-snug tracking-tight group-hover:text-brand-emerald transition-colors line-clamp-2">{{ $tb->tieu_de }}</h4>
                            </a>
                        @empty
                    ...
                    <div class="p-4 bg-slate-50/50 border-t border-slate-100">
                        <a href="{{ route('student.thongbao') }}" class="saas-btn-secondary w-full h-10 text-[10px] uppercase font-bold tracking-widest">Xem tất cả bản tin</a>
                    </div>
                </article>
                
                {{-- Support Card --}}
                <article class="saas-card bg-slate-900 border-slate-800 p-8 text-white relative overflow-hidden group">
                    <div class="absolute -right-8 -top-8 h-32 w-32 rounded-full bg-slate-800/30 blur-3xl group-hover:bg-emerald-500/10 transition-all duration-700"></div>
                    <div class="relative z-10">
                        ...
                        <div class="flex flex-col gap-1 group/line">
                            ...
                            <a href="tel:0241234567" class="text-sm font-bold tabular-nums tracking-tight hover:text-brand-emerald transition-colors text-slate-300">024.123.4567</a>
                        </div>
                    </div>
                </article>
            </aside>
