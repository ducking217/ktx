                    ...
                    <div class="divide-y divide-slate-50">
                        @forelse($thongbaoLienQuan as $tb)
                            <a href="{{ route('student.chitietthongbao', $tb->id) }}" class="group block p-5 transition-colors hover:bg-slate-50/50">
                                <time class="text-[9px] font-bold text-blue-600 uppercase tracking-widest mb-1.5 block tabular-nums">{{ $tb->created_at?->format('d/m/Y') }}</time>
                                <h4 class="text-xs font-bold text-slate-900 leading-snug tracking-tight group-hover:text-blue-600 transition-colors line-clamp-2">{{ $tb->tieu_de }}</h4>
                            </a>
                        @empty
