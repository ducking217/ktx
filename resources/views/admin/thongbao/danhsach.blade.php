<x-admin-layout>
                ...
                @forelse($thongbao as $item)
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="py-5 max-w-xs">
                            <div class="text-sm font-bold text-slate-900 leading-tight group-hover:text-brand-emerald transition-colors">{{ $item->tieu_de }}</div>
                        ...
                        <td class="py-5 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <form method="POST" action="{{ route('admin.thongbao.xoa', ['id' => $item->id]) }}" x-data="{ showConfirm: false }" @confirmed="$el.submit()" class="inline">
                                    @csrf
    ...
    @push('modals')
        <x-modal id="modal-themthongbao" title="Soạn thảo thông báo" subtitle="Thiết lập nội dung mới để phát hành tới cộng đồng cư dân.">
            <form method="POST" action="{{ route('admin.thongbao.store') }}" class="space-y-6">
                @csrf
                <div class="space-y-2">
        ...
        @foreach($thongbao as $item)
            <x-modal id="modal-suathongbao-{{ $item->id }}" title="Hiệu chỉnh thông báo" subtitle="Cập nhật lại nội dung thông báo #{{ $item->id }}.">
                <form method="POST" action="{{ route('admin.thongbao.capnhat', ['id' => $item->id]) }}" class="space-y-6">
                    @csrf
                    <div class="space-y-2">
