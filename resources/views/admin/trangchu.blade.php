<x-admin-layout>
        ...
        <article class="saas-card lg:col-span-8 p-8 flex flex-col justify-between overflow-hidden relative border-slate-200/60 shadow-sm">
            <div class="relative z-10">
                ...
                <h2 class="text-2xl font-bold tracking-tight text-slate-900 leading-tight mb-2">
                    Công suất lấp đầy: <span class="text-blue-600 tabular-nums">{{ $tyLeLapDay }}%</span>
                </h2>
            ...
            <div class="relative z-10 mt-8 grid grid-cols-3 gap-8 border-t border-slate-100 pt-6">
                <div class="space-y-3">
                    ...
                    <div class="h-1.5 w-full overflow-hidden rounded-full bg-slate-100 border border-slate-200/50">
                        <div class="h-full rounded-full bg-slate-900 transition-all duration-700" style="width: {{ $tyLeLapDay }}%"></div>
                    </div>
                </div>
                <div class="space-y-3">
                    <div class="text-[9px] font-bold uppercase tracking-widest text-slate-400">Phòng trống</div>
                    <div class="text-3xl font-bold tracking-tight text-slate-900 tabular-nums">{{ $phongTrong }}</div>
    ...
    <section class="mb-8 grid grid-cols-1 gap-4 md:grid-cols-2">
        <a href="{{ route('admin.duyetdangky') }}" class="saas-card p-5 flex items-center justify-between group hover:border-blue-200 transition-all hover:shadow-lg hover:shadow-blue-500/5 border-slate-200/60 shadow-sm">
            <div class="flex items-center gap-4">
                <div class="relative h-10 w-10 flex items-center justify-center rounded-xl bg-slate-50 text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-all duration-300 border border-slate-100 shadow-sm">
                    ...
                    @if($dangKyChoDuyet > 0)
                        <span class="absolute -top-1.5 -right-1.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-blue-600 px-1 text-[8px] font-bold text-white ring-2 ring-white">{{ $dangKyChoDuyet }}</span>
                    @endif
                </div>
                <div>
        ...
        <a href="{{ route('admin.quanlybaohong') }}" class="saas-card p-5 flex items-center justify-between group hover:border-amber-200 transition-all hover:shadow-lg hover:shadow-amber-500/5 border-slate-200/60 shadow-sm">
            <div class="flex items-center gap-4">
        ...
        <div class="space-y-6 xl:col-span-8">
            ...
            <article class="saas-card overflow-hidden border-slate-200/60 shadow-sm">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    ...
                    <a href="{{ route('admin.duyetdangky') }}" class="saas-btn-secondary h-8 px-4 text-[9px] font-bold uppercase tracking-widest">Quản lý</a>
                </div>
                        ...
                        @foreach ($listDangKy as $dangky)
                            <div class="flex items-center gap-4 px-6 py-4 hover:bg-slate-50/50 transition-colors group">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="text-xs font-bold text-slate-900 truncate group-hover:text-blue-600 transition-colors">{{ $dangky['name'] }}</div>
            ...
            <article class="saas-card overflow-hidden border-slate-200/60 shadow-sm">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    ...
                    <a href="{{ route('admin.quanlybaohong') }}" class="saas-btn-secondary h-8 px-4 text-[9px] font-bold uppercase tracking-widest">Điều phối</a>
                </div>
        ...
        <div class="space-y-6 xl:col-span-4">
                    ...
                    @foreach ($listCongSuat as $toa)
                        <div class="group cursor-default">
                            <div class="flex justify-between items-center mb-2">
                                <span class="text-xs font-bold text-slate-700 group-hover:text-blue-600 transition-colors uppercase tracking-tight">{{ $toa['name'] }}</span>
                            ...
                            <div class="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                                <div class="h-full bg-slate-800 group-hover:bg-blue-600 transition-all duration-500 rounded-full" style="width: {{ $toa['percentage'] }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <article class="saas-card overflow-hidden border-slate-200/60 shadow-sm">
                ...
                <div class="p-3 space-y-1 bg-white">
                    <a href="{{ route('admin.quanlyhoadon', ['tab' => 'cong-no']) }}" class="flex items-center justify-between p-4 rounded-xl hover:bg-slate-50 transition-all group border border-transparent hover:border-slate-100">
                    ...
                    <a href="{{ route('admin.quanlycauhinh') }}" class="flex items-center justify-between p-4 rounded-xl hover:bg-slate-50 transition-all group border border-transparent hover:border-slate-100">
                        <div class="flex items-center gap-3">
