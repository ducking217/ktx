<x-admin-layout>
    ...
    <div class="space-y-8">
        ...
        <div class="saas-card p-6 bg-slate-50/50 border-dashed">
            <form action="{{ route('admin.sinhvien.index') }}" method="GET" class="flex flex-wrap items-end gap-6">
                ...
                <div class="flex items-end gap-3">
                    ...
                    @if(request('q'))
                        <a href="{{ route('admin.sinhvien.index') }}" class="saas-btn-secondary h-11 px-5">Xóa lọc</a>
                    @endif
                </div>
            </form>
        </div>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="py-5">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <div class="text-sm font-bold text-slate-900 leading-tight group-hover:text-brand-emerald transition-colors">{{ $sinhvien->taikhoan?->name ?? 'Chưa có' }}</div>
                        ...
                        <td class="py-5 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('admin.sinhvien.chitiet', $sinhvien->id) }}" class="h-9 w-9 inline-flex items-center justify-center text-slate-400 hover:text-brand-emerald hover:bg-brand-emerald/10 border border-transparent hover:border-brand-emerald/20 rounded-xl transition-all shadow-sm hover:shadow-md" title="Xem hồ sơ">
