<x-admin-layout>
    <x-slot:title>Quản lý Hợp đồng Cư trú</x-slot:title>

    <div class="space-y-8">
        <x-admin.page-header
            title="Hệ thống hợp đồng"
            subtitle="Quản lý vòng đời hợp đồng, thực thi điều khoản pháp lý và quy trình thanh lý tài chính."
        >
            <div class="saas-card flex items-center px-4 h-10 bg-white/60 border-slate-200/60 shadow-sm">
                <form action="{{ route('admin.quanlyhopdong') }}" method="GET" class="flex items-center gap-2">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Tìm sinh viên, mã HĐ..." class="bg-transparent border-none focus:ring-0 text-sm font-medium text-slate-900 w-44 placeholder:text-slate-300" />
                </form>
            </div>
        </x-admin.page-header>
        ...
        <div class="saas-card p-6 bg-slate-50/50 border-dashed">
            <form method="GET" action="{{ route('admin.quanlyhopdong') }}" class="flex flex-wrap items-end gap-6">
            ...
            <tbody>
                @forelse($hopdong as $item)
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="py-5">
                            <span class="text-[11px] font-bold text-slate-400 tabular-nums bg-slate-50 px-2 py-0.5 rounded border border-slate-200/60 uppercase tracking-widest">{{ $item->ma_hd }}</span>
                        </td>
                        <td class="py-5">
                            ...
                            <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">{{ $item->sinhvien?->ma_sinh_vien ?? 'Chưa có' }}</div>
                        </td>
                        <td class="py-5">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-bold text-slate-700">{{ $item->giuong?->phong?->ten_phong ?? 'Chưa có' }}</span>
                            </div>
                        </td>
                        <td class="py-5 min-w-[160px]">
                            <div class="text-[11px] font-bold text-slate-900 tabular-nums">
                                {{ is_string($item->ngay_bat_dau) ? $item->ngay_bat_dau : $item->ngay_bat_dau->format('d.m.Y') }}
                                <span class="mx-1 text-slate-300">→</span>
                                {{ is_string($item->ngay_ket_thuc) ? $item->ngay_ket_thuc : $item->ngay_ket_thuc->format('d.m.Y') }}
                            </div>
                            <div class="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden mt-2">
                                ...
                                @endphp
                                <div class="bg-blue-600 h-full transition-all duration-1000 rounded-full" style="width: {{ $progress }}%"></div>
                            </div>
                        </td>
                        ...
                        <td class="py-5 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                @if($item->trang_thai == \App\Enums\ContractStatus::Active)
                                    ...
                                    <button type="button" data-modal-target="modal-thanh-ly-{{ $item->id }}" data-modal-toggle="modal-thanh-ly-{{ $item->id }}" class="h-9 w-9 inline-flex items-center justify-center text-slate-400 hover:text-rose-600 hover:bg-rose-50 border border-transparent hover:border-rose-100 rounded-xl transition-all shadow-sm hover:shadow-md" title="Thanh lý hợp đồng">
    ...
    @push('modals')
        @foreach ($hopdong as $item)
            ...
            {{-- Modal Chi tiết --}}
            <x-modal id="modal-chi-tiet-{{ $item->id }}" title="Chi tiết hợp đồng {{ $item->ma_hd }}" subtitle="Thông tin đầy đủ về hợp đồng cư trú và điều khoản liên quan.">
                <div class="space-y-6">
                    <div class="p-6 rounded-2xl bg-slate-50 border border-slate-200/60 flex items-center gap-4">
                        ...
                        <div>
                            <div class="text-base font-bold text-slate-900 leading-none">{{ $item->sinhvien?->user?->name ?? 'Chưa xác định' }}</div>
                            <div class="text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-1.5">Mã SV: {{ $item->sinhvien->ma_sinh_vien ?? 'Chưa có' }}</div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="saas-card p-5 bg-white/60 border-slate-100">
                            ...
                            <span class="text-sm font-bold text-slate-900 tabular-nums">{{ is_string($item->ngay_bat_dau) ? $item->ngay_bat_dau : $item->ngay_bat_dau->format('d.m.Y') }}</span>
                        </div>
                        <div class="saas-card p-5 bg-white/60 border-slate-100">
                            <span class="block text-[9px] font-bold uppercase tracking-widest text-slate-400 mb-2">Ngày kết thúc</span>
                            <span class="text-sm font-bold text-blue-600 tabular-nums">{{ is_string($item->ngay_ket_thuc) ? $item->ngay_ket_thuc : $item->ngay_ket_thuc->format('d.m.Y') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="divide-y divide-slate-100 saas-card px-6 border-slate-100 shadow-sm">
                        ...
                        <div class="flex items-center justify-between py-4">
                            ...
                            <span class="text-sm font-bold text-slate-900 tabular-nums">{{ number_format((int) $item->gia_thuc_te) }}<small class="ml-1 text-slate-400 uppercase font-bold text-[10px]">VNĐ</small></span>
                        </div>
                        <div class="flex items-center justify-between py-4">
                            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Trạng thái</span>
                            <span class="saas-badge {{ $statusBadge }}">{{ $item->trang_thai->label() }}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-4 pt-2">
                        <a href="{{ route('admin.hopdong.pdf', $item->id) }}" target="_blank" class="saas-btn-primary flex-1 h-11 justify-center shadow-lg shadow-blue-500/20">
                        ...
                        <button type="button" data-modal-hide="modal-chi-tiet-{{ $item->id }}" class="saas-btn-secondary flex-1 h-11 justify-center">Đóng</button>
                    </div>
                </div>
            </x-modal>

            {{-- Modal Gia hạn --}}
            <x-modal id="modal-gia-han-{{ $item->id }}" title="Gia hạn hợp đồng" subtitle="Cập nhật ngày kết thúc mới cho hợp đồng cư trú {{ $item->ma_hd }}.">
                <form action="{{ route('admin.hopdong.giahan', $item->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        @endphp
                        <input name="ngay_ket_thuc" type="date" value="{{ old('ngay_ket_thuc', $ngayMacDinh->format('Y-m-d')) }}" class="saas-input h-11 font-bold tabular-nums" required />
                        <x-input-error :messages="$errors->get('ngay_ket_thuc')" class="mt-2 ml-1" />
                    </div>
            ...
            {{-- Modal Thanh lý --}}
            <x-modal id="modal-thanh-ly-{{ $item->id }}" title="Thanh lý hợp đồng" subtitle="Xác nhận chấm dứt hợp đồng {{ $item->ma_hd }} và thực hiện đối soát tài chính.">
                <form action="{{ route('admin.hopdong.thanhly', $item->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="h-11 saas-input bg-slate-50/50 flex items-center font-bold text-slate-900 tabular-nums border-slate-200/60 shadow-inner px-4">
                            {{ number_format((int) $tienCoc) }}<span class="ml-1 text-slate-400 font-bold uppercase text-[10px]">VNĐ</span>
                        </div>
                    </div>

                    <div class="space-y-2">
                        <label for="phi_hu_hai_{{ $item->id }}" class="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-1">Phí hư hại / vi phạm</label>
                        <div class="relative group">
                            <input name="phi_hu_hai" id="phi_hu_hai_{{ $item->id }}" type="number" min="0" value="0" class="saas-input h-11 pl-14 font-bold tabular-nums focus:border-rose-300 transition-all" />
