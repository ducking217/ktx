...
<head>
    ...
    <title>{{ $pageTitle }} - Admin KTX</title>
    ...
    <div class="saas-layout">
        ...
        <nav class="fixed bottom-0 left-0 right-0 z-40 bg-white/90 backdrop-blur-lg border-t border-slate-200 p-2 lg:hidden">
            <div class="flex items-center justify-around max-w-md mx-auto">
                <a href="{{ route('admin.trangchu') }}" class="flex flex-col items-center gap-1 p-2 rounded-lg transition-colors {{ request()->routeIs('admin.trangchu') ? 'text-blue-600 bg-blue-50' : 'text-slate-500 hover:text-slate-900' }}">
                ...
                <a href="{{ route('admin.quanlyhoadon') }}" class="flex flex-col items-center gap-1 p-2 rounded-lg transition-colors {{ request()->routeIs('admin.quanlyhoadon') ? 'text-blue-600 bg-blue-50' : 'text-slate-500 hover:text-slate-900' }}">
                ...
                <a href="{{ route('admin.quanlybaohong') }}" class="flex flex-col items-center gap-1 p-2 rounded-lg transition-colors {{ request()->routeIs('admin.quanlybaohong') ? 'text-blue-600 bg-blue-50' : 'text-slate-500 hover:text-slate-900' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    <span class="text-[10px] font-medium">Issues</span>
                </a>
                <a href="{{ route('admin.quanlysinhvien') }}" class="flex flex-col items-center gap-1 p-2 rounded-lg transition-colors {{ request()->routeIs('admin.quanlysinhvien') ? 'text-blue-600 bg-blue-50' : 'text-slate-500 hover:text-slate-900' }}">
