...
- Memory: Luôn đọc `memory-bank/` trước khi làm. Chỉ cập nhật `progress.md` khi task có thay đổi feature/bugfix (không cập nhật cho thao tác tooling như index/serve).
...
- **Preflight (bắt buộc):** Trước khi dùng bất kỳ GitNexus tool/MCP nào: kiểm tra index `gitnexus status`. Nếu stale/thiếu index: chạy `gitnexus analyze --force --name ktx` tại project root.
- **Context (bắt buộc):** Luôn đọc `AGENTS.md` và (khi cần) `gitnexus://repo/ktx/context` để nắm Functional Clusters + Execution Flows + độ mới của index.
- **Deep Exploration (ưu tiên hơn grep):**
  - Dò khái niệm/luồng nghiệp vụ: `gitnexus_query({query: "<concept>"})` → chọn process liên quan → đọc `gitnexus://repo/ktx/process/<name>`.
  - Dò symbol chuẩn trước khi sửa: `gitnexus_context({name: "<symbolName>"})` để thấy callers/callees/processes.
- **Impact Analysis (MANDATORY):** Trước khi sửa bất kỳ function/class/method: chạy `gitnexus_impact({target: "<symbolName>", direction: "upstream"})`. Nếu HIGH/CRITICAL: phải dừng và báo bán kính ảnh hưởng (callers, processes, rủi ro) trước khi đụng code.
- **Refactor/Rename:** Không find-and-replace. Dùng `gitnexus_rename` để rename an toàn theo call graph.
- **Verification (bắt buộc sau khi sửa code):**
  - Chạy `gitnexus_detect_changes()` để xác nhận phạm vi ảnh hưởng khớp dự kiến.
  - Với thay đổi logic: chạy test/lint phù hợp (phpunit/pint/phpstan) theo phạm vi thay đổi. Mọi lệnh Artisan/Composer/NPM phải chạy qua @openspace.
- **Laravel hotspots (cảnh báo side effects):** Trước khi chạm vào các luồng dưới đây, luôn trace process/symbol để tránh side effects từ Observer/Notification/Mail/Transaction:
  - `DangkyService::duyetDangKy`, `DangkyService::xacNhanThanhToan`
  - Workflow hóa đơn `pending_confirmation` và đối soát thanh toán
  - Private file access `/private-files?path=...` (signed URL)
  - Các Observer trong `app/Observers/**` (có thể mutate trạng thái/ghi nhật ký/dispatch notification)

## GitNexus on Windows (thực chiến)
- PowerShell 5 không hỗ trợ `&&` để chain lệnh: dùng `;` hoặc chạy 2 lệnh riêng.
- Nếu `gitnexus serve` báo port bị chiếm (EADDRINUSE): chạy `gitnexus serve --port 4748` (hoặc port khác).
- Nếu index lỗi WAL/corrupted (vd: `Corrupted wal file` / `Analysis did not finalize`):
  - `gitnexus clean -f`
  - `gitnexus analyze --force --name ktx`
...
Tuyệt đối không thay đổi logic nền tảng nếu GitNexus báo cáo "Upstream impact" quá lớn mà chưa có sự đồng ý của người dùng.
