                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $danhsachphong; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $phong): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            <td class="py-5">
                                <div class="text-sm font-bold text-slate-900 leading-tight group-hover:text-brand-emerald transition-colors"><?php echo e($phong->ten_phong); ?></div>
                            ...
                            <td class="py-5 min-w-[150px]">
                                <div class="flex items-center gap-3">
                                    <div class="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                        <div class="h-full rounded-full transition-all duration-700 <?php echo e($daydu ? 'bg-slate-900' : 'bg-brand-emerald'); ?>" style="<?php echo \Illuminate\Support\Arr::toCssStyles(["width: $phantram%"]) ?>"></div>
                                    </div>
                            ...
                            <td class="py-5 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="<?php echo e(route('admin.phong.chitiet', ['id' => $phong->id])); ?>" class="h-9 w-9 inline-flex items-center justify-center text-slate-400 hover:text-brand-emerald hover:bg-brand-emerald/10 border border-transparent hover:border-brand-emerald/20 rounded-xl transition-all shadow-sm hover:shadow-md" title="Chi tiết cư trú">
                                    ...
                                    <form method="POST" action="<?php echo e(route('admin.phong.destroy', ['id' => $phong->id])); ?>" onsubmit="return confirm('Xác nhận loại bỏ hoàn toàn phòng này khỏi hệ thống?')" class="inline">
            ...
            <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
                <?php $__empty_1 = true; $__currentLoopData = $danhsachphong; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $phong): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    ...
                    <article class="saas-card p-6 relative overflow-hidden group hover:border-emerald-200 transition-all hover:shadow-xl hover:shadow-emerald-500/5">
                        <div class="absolute top-0 right-0 h-1.5 w-full <?php echo e($daydu ? 'bg-slate-900' : ($isFemaleRoom ? 'bg-rose-500' : ($isMaleRoom ? 'bg-slate-500' : 'bg-emerald-500'))); ?>"></div>
                        
                        <header class="flex items-start justify-between mb-10">
                            <div>
                                <h3 class="text-4xl font-bold text-slate-900 leading-none tabular-nums group-hover:text-brand-emerald transition-colors"><?php echo e($phong->ten_phong); ?></h3>
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <div class="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                                    <div class="h-full rounded-full transition-all duration-1000 <?php echo e($daydu ? 'bg-slate-900' : 'bg-brand-emerald'); ?>" style="<?php echo \Illuminate\Support\Arr::toCssStyles(["width: $phantram%"]) ?>"></div>
                                </div>
                            </div>
                        ...
                        <div class="mt-8 flex items-center justify-end gap-1.5">
                            ...
                            <form method="POST" action="<?php echo e(route('admin.phong.destroy', ['id' => $phong->id])); ?>" onsubmit="return confirm('Xác nhận loại bỏ phòng?')" class="inline">
                                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                <button type="submit" class="h-9 w-9 inline-flex items-center justify-center text-slate-400 hover:text-rose-600 hover:bg-rose-50 border border-transparent hover:border-rose-100 rounded-xl transition-all shadow-sm hover:shadow-md" title="Loại bỏ">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M9 7h6m-7 0V5a2 2 0 012-2h2a2 2 0 012 2v2"/></svg>
                                </button>
...
<?php $component->withAttributes(['id' => 'modal-themphong','title' => 'Thêm phòng mới','subtitle' => 'Khởi tạo thực thể phòng mới trong cơ sở hạ tầng KTX.']); ?>
            <form method="POST" action="<?php echo e(route('admin.phong.store')); ?>" class="space-y-6">
                <?php echo csrf_field(); ?>
...
<?php $component->withAttributes(['id' => 'modal-gan-taisan','title' => 'Thêm tài sản (gán hàng loạt)','subtitle' => 'Nhập loại tài sản và gán cho toàn bộ phòng trong 1 tòa, hoặc gán cho 1 phòng cụ thể.']); ?>
            <form method="POST" action="<?php echo e(route('admin.taisan.gan_hang_loat')); ?>" class="space-y-6" x-data="{ phamVi: '<?php echo e(old('pham_vi', 'toa')); ?>' }">
                ...
                <label class="flex items-center gap-3 rounded-2xl bg-slate-50 p-4 border border-slate-100">
                    <input type="checkbox" name="cong_don" value="1" class="h-4 w-4 rounded border-slate-300 text-brand-emerald focus:ring-brand-emerald" <?php if(old('cong_don', '1') == '1'): echo 'checked'; endif; ?>>
                    <div class="min-w-0">
...
<?php $component->withAttributes(['id' => 'modal-capnhatphong-'.e($phong->id).'','title' => 'Cập nhật phòng','subtitle' => 'Thay đổi thông số vận hành và quy định cho phòng '.e($phong->ten_phong).'.']); ?>
                <form method="POST" action="<?php echo e(route('admin.phong.update', ['id' => $phong->id])); ?>" class="space-y-6">
                    <?php echo csrf_field(); ?>
