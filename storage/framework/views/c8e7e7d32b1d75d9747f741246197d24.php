...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('title', null, []); ?> Bảng điều khiển <?php $__env->endSlot(); ?>
    ...
    <section class="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <article class="saas-card lg:col-span-8 p-8 flex flex-col justify-between overflow-hidden relative">
            <div class="relative z-10">
                ...
                <p class="text-xs text-slate-400 font-medium max-w-xl leading-relaxed">
                    ...
                    còn <span class="text-slate-700 font-bold tabular-nums"><?php echo e($phongTrong); ?></span> phòng trống,
                    <span class="text-slate-700 font-bold tabular-nums"><?php echo e($phongBaoTri); ?></span> đang bảo trì.
                </p>
            </div>
            ...
            <div class="relative z-10 mt-8 grid grid-cols-3 gap-8 border-t border-slate-100 pt-6">
                <div class="space-y-3">
                    ...
                    <div class="flex items-baseline gap-1">
                        <span class="text-3xl font-bold tracking-tight text-slate-900 tabular-nums"><?php echo e($tyLeLapDay); ?></span>
                    ...
                    <div class="h-1.5 w-full overflow-hidden rounded-full bg-slate-100 border border-slate-200/50">
                        <div class="h-full rounded-full bg-brand-emerald transition-all duration-700" style="--width: <?php echo e($tyLeLapDay); ?>%; width: var(--width);"></div>
                    </div>
                </div>
                <div class="space-y-3">
                ...
                <div class="space-y-3">
                    ...
                    <div class="text-3xl font-bold tracking-tight text-slate-900 tabular-nums"><?php echo e($phongBaoTri); ?></div>
        ...
        <article class="saas-card lg:col-span-4 p-8 flex flex-col justify-between overflow-hidden">
            <div>
                <div class="flex items-start justify-between mb-4">
                    <div>
                        ...
                        <div class="text-2xl font-bold tracking-tight text-slate-900 tabular-nums">
                            <?php echo e(number_format($doanhThuThangNay)); ?><span class="ml-1 text-sm font-bold text-slate-300">đ</span>
                        </div>
                    </div>
                ...
                <div class="inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[9px] font-bold border <?php echo e($chenhLechDoanhThu >= 0 ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-rose-50 text-rose-700 border-rose-100'); ?>">
                    <svg class="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="<?php echo e($chenhLechDoanhThu >= 0 ? 'M5 10l7-7 7 7' : 'M19 14l-7 7-7-7'); ?>"/></svg>
                    <?php echo e(abs($tyLeDoanhThu)); ?>% so với tháng trước
                </div>
            </div>

            <div class="mt-6">
                <div class="flex items-end gap-1.5 h-16 mb-3">
                    <?php $__currentLoopData = $xuHuongDoanhThu; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="flex-1 rounded-md bg-slate-100 hover:bg-slate-800 transition-all cursor-default relative group/bar" style="--height: <?php echo e($item['height']); ?>%; height: var(--height);">
                            <div class="invisible absolute -top-9 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[9px] px-2 py-1 rounded-lg opacity-0 group-hover/bar:visible group-hover/bar:opacity-100 transition-all whitespace-nowrap z-20 font-bold tabular-nums">
                                <?php echo e(number_format($item['value'])); ?>đ
                            </div>
                        </div>
        ...
        <a href="<?php echo e(route('admin.baohong.index')); ?>" class="saas-card p-5 flex items-center justify-between group hover:border-amber-200 transition-all hover:shadow-lg hover:shadow-amber-500/5">
            <div class="flex items-center gap-4">
                <div class="relative h-10 w-10 flex items-center justify-center rounded-xl bg-slate-50 text-slate-400 group-hover:bg-amber-500 group-hover:text-white transition-all duration-300 border border-slate-100 shadow-sm">
                    ...
                    <?php if($suCoMo > 0): ?>
                        <span class="absolute -top-1.5 -right-1.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-amber-500 px-1 text-[8px] font-bold text-white ring-2 ring-white"><?php echo e($suCoMo); ?></span>
                    <?php endif; ?>
                </div>
                <div>
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-12">
        <div class="space-y-6 xl:col-span-8">
            ...
            <article class="saas-card overflow-hidden">
                ...
                <?php else: ?>
                    <div class="divide-y divide-slate-100">
                        <?php $__currentLoopData = $listDangKy; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dangky): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="flex items-center gap-4 px-6 py-4 hover:bg-slate-50/50 transition-colors group">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5 flex items-center gap-1.5">
                                        ...
                                        <span class="tabular-nums"><?php echo e($dangky['time']); ?></span>
                                    </div>
                                </div>
                                <span class="saas-badge <?php echo e(str_contains($dangky['statusClass'], 'success') ? 'saas-badge-success' : 'saas-badge-info'); ?> text-[8px] px-2.5 py-0.5"><?php echo e($dangky['statusLabel']); ?></span>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                <?php endif; ?>
            ...
            <article class="saas-card overflow-hidden">
                ...
                <div class="divide-y divide-slate-100">
                    <?php $__empty_1 = true; $__currentLoopData = $listBaoHong; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $baohong): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="flex items-center justify-between px-6 py-4 hover:bg-slate-50/50 transition-colors group">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <div class="text-xs font-bold text-slate-900 group-hover:text-amber-600 transition-colors leading-tight"><?php echo e($baohong['mota']); ?></div>
                                    <div class="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5 flex items-center gap-1.5">
                                        <span class="text-slate-600">Phòng <?php echo e($baohong['phongName']); ?></span>
                                        <span class="h-0.5 w-0.5 rounded-full bg-slate-300"></span>
                                        <span class="tabular-nums"><?php echo e($baohong['time']); ?></span>
                                    </div>
                                </div>
                            </div>
                            <span class="saas-badge saas-badge-warning text-[8px] px-2.5 py-0.5"><?php echo e($baohong['statusLabel']); ?></span>
                        </div>
                    ...
                    <?php $__currentLoopData = $listCongSuat; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $toa): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="group cursor-default">
                            ...
                            <div class="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                                <div class="h-full bg-slate-800 group-hover:bg-brand-emerald transition-all duration-500 rounded-full" style="--width: <?php echo e($toa['percentage']); ?>%; width: var(--width);"></div>
                            </div>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
...
<?php endif; ?>
<?php /**PATH D:\laragon\www\hethongquanlyktxv1\resources\views/admin/trangchu.blade.php ENDPATH**/ ?>