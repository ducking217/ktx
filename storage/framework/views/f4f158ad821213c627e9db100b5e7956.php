...
<?php $__env->startSection('noidung'); ?>
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <div class="text-xl font-semibold text-slate-900">
                    <?php echo e($isRefund ? 'Hoàn cọc' : ($isDeposit ? 'Tiền cọc' : ($isExtra ? 'Phát sinh' : ('Hóa đơn ' . $kyHienThi)))); ?>

                </div>
                <div class="mt-0.5 text-sm text-slate-500">
                    Mã: <span class="font-semibold text-slate-700 tabular-nums"><?php echo e($maHoaDon); ?></span> • Phòng <?php echo e($tenPhong); ?>
        ...
        <?php else: ?>
            <div class="saas-card p-5">
                <div class="font-semibold text-slate-900">
                    <?php echo e($hoadon->trang_thai === \App\Enums\InvoiceStatus::Overdue ? 'Hóa đơn đã quá hạn' : 'Hóa đơn chưa thanh toán'); ?>
                ...
                <?php if($hoadon->giao_dich_tu_choi_gan_nhat): ?>
                    <?php
                    ...
                    <div class="mt-4 rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">
                        ...
                        <div class="mt-1">
                            <?php echo e($lyDoTuChoi); ?>

                        </div>
                        <div class="mt-2 text-[11px] font-semibold text-rose-700/80">
                            Thời gian đối soát: <?php echo e($hoadon->giao_dich_tu_choi_gan_nhat->updated_at?->format('d/m/Y H:i') ?? ($hoadon->giao_dich_tu_choi_gan_nhat->ngay_giao_dich?->format('d/m/Y H:i') ?? '—')); ?>
    ...
    <?php if(!empty($thongBaoNhacNo) && count($thongBaoNhacNo) > 0): ?>
        <?php
            $nhacNoMoiNhat = $thongBaoNhacNo[0] ?? null;
        ?>
        <?php if($nhacNoMoiNhat): ?>
            <div class="mb-6 saas-card p-5">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="text-sm text-slate-600">
                        <?php echo e($nhacNoMoiNhat->noi_dung); ?>
    ...
    <?php if(!$isRefund && in_array($hoadon->trang_thai, [\App\Enums\InvoiceStatus::Unpaid, \App\Enums\InvoiceStatus::Overdue], true)): ?>
        <div id="huong-dan-thanh-toan" class="mb-6 saas-card p-6" x-data="{ copied: null, async copy(key, text) { try { await navigator.clipboard.writeText(text); this.copied = key; setTimeout(() => this.copied = null, 1200); } catch (e) { this.copied = null; } } }">
            <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                ...
                <div class="text-right">
                    ...
                    <div class="text-xl font-semibold text-slate-900 tabular-nums"><?php echo e(number_format((int) ($thanhToan['so_tien'] ?? $hoadon->tong_tien))); ?> đ</div>
                </div>
            </div>
            ...
            <div class="mt-5 grid gap-4 md:grid-cols-3">
                <div class="rounded-lg border border-slate-200/60 bg-slate-50 p-4">
                    ...
                    <div class="mt-1 font-semibold text-slate-900"><?php echo e($bankName); ?></div>
                </div>
                <div class="rounded-lg border border-slate-200/60 bg-slate-50 p-4">
                    <div class="saas-label !mb-0">Số tài khoản</div>
                    <div class="mt-1 flex items-start justify-between gap-3">
                        <div>
                            <div class="font-semibold text-slate-900 tabular-nums"><?php echo e($accountNo); ?></div>
                            <div class="mt-0.5 text-xs text-slate-500"><?php echo e($accountName); ?></div>
                        </div>
                        <div class="text-right">
                            <button type="button" class="saas-btn-secondary h-9 px-3 text-xs font-semibold" @click="copy('stk', <?php echo \Illuminate\Support\Js::from($accountNo)->toHtml() ?>)">
                                Sao chép
                            </button>
                ...
                <div class="rounded-lg border border-slate-200/60 bg-slate-50 p-4">
                    ...
                    <div class="mt-1 flex items-start justify-between gap-3">
                        <div class="font-semibold text-slate-900 break-all"><?php echo e($transferContent); ?></div>
                        <div class="text-right shrink-0">
                            <button type="button" class="saas-btn-secondary h-9 px-3 text-xs font-semibold" @click="copy('ndck', <?php echo \Illuminate\Support\Js::from($transferContent)->toHtml() ?>)">
                                Sao chép
                            </button>
            ...
            <form method="POST" action="<?php echo e(route('student.hoadon.yeu_cau_xac_nhan', $hoadon->id)); ?>" class="mt-6 rounded-lg border border-slate-200/60 bg-slate-50 p-4">
                ...
                <div class="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div class="text-xs text-slate-500">
                        <?php if(!empty($thanhToan['so_dien_thoai_lien_he'])): ?>
                            Hotline: <span class="font-semibold text-slate-900"><?php echo e($thanhToan['so_dien_thoai_lien_he']); ?></span>
                        <?php endif; ?>
                        <?php if(!empty($thanhToan['email_lien_he'])): ?>
                            <?php if(!empty($thanhToan['so_dien_thoai_lien_he'])): ?> • <?php endif; ?>
                            Email: <span class="font-semibold text-slate-900"><?php echo e($thanhToan['email_lien_he']); ?></span>
                        <?php endif; ?>
                    </div>
    ...
    <div class="mb-6 saas-card p-6">
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div class="text-right">
                ...
                <div class="text-xl font-semibold text-slate-900 tabular-nums"><?php echo e(number_format((int) $hoadon->tong_tien)); ?> đ</div>
            </div>
        </div>
                    ...
                    <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="grid grid-cols-1 gap-2 px-4 py-4 md:grid-cols-12 md:items-start md:gap-0">
                            <div class="md:col-span-4">
                                <div class="font-semibold text-slate-900"><?php echo e($row['ten']); ?></div>
                            </div>
                            <div class="md:col-span-2 md:text-right">
                                <div class="font-semibold text-slate-900 tabular-nums"><?php echo e(number_format((int) $row['so_tien'])); ?> đ</div>
                                <?php if(!$isRefund && !$isDeposit && !$isExtra && (int) $soNguoiTrongPhong > 1): ?>
                                    <div class="mt-0.5 text-[11px] text-slate-500">≈ <?php echo e(number_format((int) round(((int) $row['so_tien']) / max(1, (int) $soNguoiTrongPhong)))); ?> đ/người</div>
                                <?php endif; ?>
                            </div>
                            <div class="md:col-span-2">
                                <div class="text-[11px] font-semibold text-slate-900"><?php echo e($row['don_vi']); ?></div>
                            </div>
                            <div class="md:col-span-2">
                                <div class="text-[11px] font-semibold text-slate-900"><?php echo e($row['thoi_gian']); ?></div>
                            </div>
                            <div class="md:col-span-2">
                                <div class="text-[11px] text-slate-600 leading-relaxed"><?php echo e($row['mo_ta']); ?></div>
                            </div>
                        </div>
...
<?php echo $__env->make('student.layouts.chinh', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\laragon\www\hethongquanlyktxv1\resources\views/student/phongcuatoi/chiTietHoaDon.blade.php ENDPATH**/ ?>